...
from code_migration.config import EXAMPLES
from code_migration.data import generate_mock_data
from code_migration.pipeline import compare, convert, detect, reference_runner
from code_migration.pipeline.safety import UnsafeCodeError, validate_transform_code
...
def test_rule_convert_produces_valid_transform():
    # sql + dbt -> a syntactically valid module defining transform(spark) via spark.sql.
    # (compile, not exec — exec'ing would import pyspark, which is an optional dep.)
    import ast
    ...
    for key, lang in (("sql", "sql"), ("dbt", "dbt")):
        ...
        assert any(isinstance(n, ast.FunctionDef) and n.name == "transform" for n in ast.walk(tree))
...
def test_candidate_runs_on_local_spark():
    ...
    """
    from code_migration.pipeline import databricks_runner
